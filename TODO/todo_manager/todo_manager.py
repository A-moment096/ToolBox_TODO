...
class TodoManager:
    ...
    def order_task(self, list_name: str, old_position: int, new_position: int) -> None:
        ...
        if not (1 <= new_position <= len(tasks)):
            print(f"Invalid new task number {new_position} in list {validated_list_name}.")
            return
        
        task = tasks.pop(old_position - 1)
        tasks.insert(new_position - 1, task)
        print(f"Moved task '{task}' from position {old_position} to {new_position} in list '{validated_list_name}'.")
    
    def _move_task(self, source: Section, target: Section, list_name: str, task_number: int) -> None:
    ...
    def clear_done_list(self, force: bool = False) -> None:
        ...
        if force:
            ...
            if select_option("Are you sure to delete all the done tasks and done lists?"):
                self.done_lists = {}
                print("Cleared.")
            else:
    ...
    def edit_file(self, editor: str = "") -> None:
        ...
        editor_to_use = editor or (self.config_manager.get_editor() if self.config_manager else "")
